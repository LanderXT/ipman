...
/* Sensitive files that must never reach git history. */
static const char * const k_gitignore_entries[] = {
    ".ipman/ipman.db",
    ".ipman/keysalt",
    ".ipman/ipman.db.bak",
};
static const size_t k_gitignore_entries_count =
    sizeof k_gitignore_entries / sizeof k_gitignore_entries[0];

/*
 * Derive the absolute repo-root from the (possibly relative) home path.
 * home is the .ipman directory; repo_root_out receives its parent.
 */
static int home_to_repo_root(const char *home, char *out, size_t outlen) {
    char abs[PATH_MAX];
    if (home[0] == '/') {
        int n = snprintf(abs, sizeof abs, "%s", home);
    ...
    char buf[PATH_MAX];
    int n = snprintf(buf, sizeof buf, "%s", abs);
    if (n < 0 || (size_t)n >= sizeof buf) return -1;
    char *parent = dirname(buf);
    if (parent == NULL) return -1;
    n = snprintf(out, outlen, "%s", parent);
    if (n < 0 || (size_t)n >= outlen) return -1;
    return 0;
}

static int gitignore_line_present(const char *content, const char *entry) {
    ...
    while (*p) {
        ...
        while (llen > 0 && (line[llen-1] == ' ' || line[llen-1] == '\t' ||
                             line[llen-1] == '\r'))
            llen--;
        if (llen == elen && memcmp(line, entry, elen) == 0) return 1;
...
/*
 * Appends any missing ipman entries to <repo_root>/.gitignore.
 * Best-effort: failures are silent (non-fatal to init).
 * *added_out is set to the number of entries written.
 */
static int gitignore_ensure_ipman_entries(const char *repo_root, int *added_out) {
    *added_out = 0;
    char path[PATH_MAX];
    int n = snprintf(path, sizeof path, "%s/.gitignore", repo_root);
    ...
    int n_missing = 0;
    for (size_t i = 0; i < k_gitignore_entries_count; i++) {
        if (!content || !gitignore_line_present(content, k_gitignore_entries[i]))
            missing[n_missing++] = k_gitignore_entries[i];
    }
    free(content);
...
#define IPMAN_CLAUDE_MARKER "<!-- ipman-managed -->"

/* Appended once to .claude/CLAUDE.md; idempotent on the marker. */
static const char k_ipman_claude_section[] =
    "\n" IPMAN_CLAUDE_MARKER "\n"
    "## ipman workspace\n\n"
...
/*
 * Ensures the ipman section is present in <repo_root>/.claude/CLAUDE.md.
 * Creates the file and .claude/ directory if absent.
 * Best-effort: failures are non-fatal to init.
 */
static int claude_md_ensure_ipman_section(const char *repo_root) {
    char dir[PATH_MAX];
    int n = snprintf(dir, sizeof dir, "%s/.claude", repo_root);
    if (n < 0 || (size_t)n >= sizeof dir) return -1;
    mkdir(dir, 0755);  /* ignore failure — dir may already exist */

    char path[PATH_MAX];
    n = snprintf(path, sizeof path, "%s/.claude/CLAUDE.md", repo_root);
    if (n < 0 || (size_t)n >= sizeof path) return -1;
    ...
    if (f) {
        ...
        if (sz > 0) {
            ...
            content[sz] = '\0';
            fclose(f);
            int present = strstr(content, IPMAN_CLAUDE_MARKER) != NULL;
            free(content);
            if (present) return 0;
        } else {
            fclose(f);
    ...
    f = fopen(path, "a");
    if (!f) return -1;
    fputs(k_ipman_claude_section, f);
    fclose(f);
    return 0;
}
...
static int run_init(void) {
    ...
    /* Protect sensitive workspace files from being committed.
     * Best-effort: permission errors here should not abort init. */
    char repo_root[PATH_MAX];
    int gitignore_added = 0;
    if (home_to_repo_root(home, repo_root, sizeof repo_root) == 0) {
        (void)gitignore_ensure_ipman_entries(repo_root, &gitignore_added);
        (void)claude_md_ensure_ipman_section(repo_root);
    }
    ...
    if (result != NULL) {
        ...
        cJSON_AddBoolToObject(result, "gitignore_updated", gitignore_added > 0);

        /* Always surface the list so any agent can verify the invariant,
         * regardless of whether this run added entries or found them already. */
        cJSON *instructions = cJSON_CreateArray();
        cJSON *step = cJSON_CreateObject();
        if (instructions != NULL && step != NULL) {
            cJSON_AddStringToObject(step, "action", "verify_gitignore");
            cJSON *entries = cJSON_CreateArray();
            if (entries != NULL) {
                for (size_t i = 0; i < k_gitignore_entries_count; i++)
                    cJSON_AddItemToArray(entries,
                                        cJSON_CreateString(k_gitignore_entries[i]));
                cJSON_AddItemToObject(step, "entries", entries);
            }
            cJSON_AddStringToObject(step, "note",
                "ipman init manages these .gitignore entries automatically; "
                "verify before the first commit");
            cJSON_AddItemToArray(instructions, step);
            cJSON_AddItemToObject(result, "agent_instructions", instructions);
        } else {
