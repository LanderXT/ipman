...
static const OperationSpec k_operation_specs[] = {
    ...
    { "task.close", "task", "close", "Close a task as completed with closure memory.", "id; outcome_summary; closing_comment", "lessons_learned, open_items_summary, followup_needed, commit_sha, dirty, files_changed", "closure text must be non-empty; followup_needed and dirty are boolean; commit_sha must be non-empty when provided; files_changed must be an array of strings or null.", "Task must exist, be eligible to move to done, and not already have a resolution.", "Sets status done, resolution completed, writes closure record (including any commit_sha / dirty / files_changed fields supplied by the client), emits task_closed.", "validation_failed, not_found, conflict, internal_error.", "result.task.", "closure.get, task.reopen", "{\"id\":1,\"outcome_summary\":\"Implemented and tested.\",\"closing_comment\":\"Ready for review.\",\"commit_sha\":\"abc1234\",\"dirty\":false,\"files_changed\":[\"src/foo.c\"]}", "\"outcome_summary\",\"closing_comment\"", "task" },
