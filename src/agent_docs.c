...
static const OperationSpec k_operation_specs[] = {
    ...
    { "env_var.add", "env_var", "create/bootstrap", "Register an environment variable required by the project.", "name, purpose", "example, required, sensitive", "name must match ^[A-Z_][A-Z0-9_]*$; purpose must be non-empty; required defaults to true; sensitive defaults to false. ipman never stores the variable's actual value, only its name and metadata.", "The active row must use a unique name.", "Creates an env_var and emits env_var_added on the project.", "validation_failed, conflict, internal_error.", "result.env_var.", "env_var.list, env_var.update, env_var.invalidate", "{\"name\":\"DATABASE_URL\",\"purpose\":\"primary postgres connection\",\"example\":\"postgres://localhost:5432/dev\"}", "\"name\",\"purpose\"", "env_var" },
    { "env_var.invalidate", "env_var", "ongoing execution", "Soft-invalidate an env_var while preserving audit history.", "id", "none", "id must be a positive integer.", "The env_var must exist and not already be invalidated.", "Sets invalidated fields and emits env_var_invalidated on the project.", "validation_failed, not_found, conflict, internal_error.", "result.env_var.", "env_var.list, env_var.update", "{\"id\":1}", "\"id\"", "env_var" },
    { "env_var.list", "env_var", "inspect", "List the env_vars required by the project. Sensitive examples are masked unless reveal=true.", "none", "include_invalidated, reveal", "include_invalidated and reveal must be boolean. When reveal is false, rows with sensitive=true return example=\"[sensitive]\" or null.", "none.", "No business data changes.", "validation_failed, internal_error.", "result.env_vars, result.include_invalidated, result.reveal, result.has_more, result.total_count.", "env_var.add, env_var.update, env_var.invalidate", "{\"include_invalidated\":false}", "", "env_vars, include_invalidated, reveal, has_more, total_count" },
    { "env_var.update", "env_var", "ongoing execution", "Update one or more fields of an active env_var.", "id; plus at least one mutable field", "name, purpose, example, required, sensitive", "id must be positive; name, when provided, must match ^[A-Z_][A-Z0-9_]*$; purpose and example must be non-empty when provided; required and sensitive must be boolean. Omitted fields are left unchanged.", "The env_var must exist and not be invalidated.", "Updates env_var fields and emits env_var_updated on the project.", "validation_failed, not_found, conflict, internal_error.", "result.env_var.", "env_var.list, env_var.invalidate", "{\"id\":1,\"sensitive\":true,\"purpose\":\"updated description\"}", "\"id\"", "env_var" },
    ...
    { "instruction.add", "instruction", "ongoing execution", "Add durable guidance to a plan, phase, task, or the project itself.", "entity_type, entity_id, body", "instruction_type, priority", "entity_type must be plan, phase, task, or project; entity_id must be positive; for project, entity_id must be 1 (single-row table); body and instruction_type must be non-empty strings when provided; priority must be 'critical' or 'normal' (default 'normal') and is only accepted when entity_type is 'project'.", "The referenced entity must exist.", "Creates an instruction and emits instruction_added on the referenced entity.", "validation_failed, not_found, internal_error.", "result.instruction.", "instruction.list, instruction.update, instruction.invalidate", "{\"entity_type\":\"project\",\"entity_id\":1,\"body\":\"All user-facing text must be in Spanish.\",\"instruction_type\":\"constraint\",\"priority\":\"critical\"}", "\"entity_type\",\"entity_id\",\"body\"", "instruction" },
    ...
    { "instruction.list", "instruction", "inspect", "List active guidance attached to a plan, phase, task, or the project itself.", "entity_type, entity_id", "include_invalidated", "entity_type must be plan, phase, task, or project; for project, entity_id must be 1; include_invalidated must be boolean.", "The referenced entity must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "result.instructions, result.include_invalidated, result.has_more, result.total_count.", "instruction.add, instruction.update, instruction.invalidate", "{\"entity_type\":\"project\",\"entity_id\":1}", "\"entity_type\",\"entity_id\"", "instructions, include_invalidated, has_more, total_count" },
    { "instruction.update", "instruction", "ongoing execution", "Update the text, type, and/or priority of an active instruction.", "id, body", "instruction_type, priority", "id must be positive; body must be non-empty; instruction_type must be non-empty when provided; priority must be 'critical' or 'normal' when provided and is only accepted for project-scoped instructions.", "The instruction must exist and not be invalidated.", "Updates instruction fields and emits instruction_updated on the referenced entity.", "validation_failed, not_found, conflict, internal_error.", "result.instruction.", "instruction.list, instruction.invalidate", "{\"id\":1,\"body\":\"Keep compatibility, but prefer simpler adapters.\",\"instruction_type\":\"guidance\"}", "\"id\",\"body\"", "instruction" },
    { "phase.close", "phase", "close", "Close a phase with closure memory.", "id; outcome_summary; closing_comment", "lessons_learned, open_items_summary, followup_needed, outcome", "outcome is completed or canceled; closure text must be non-empty; followup_needed is boolean.", "Phase must exist and all tasks in it must be terminal.", "Updates phase, emits phase_closed, writes closure record.", "validation_failed, not_found, conflict, internal_error.", "result.phase.", "phase.reopen, closure.get, phase.history", "{\"id\":1,\"outcome_summary\":\"Phase objectives met.\",\"closing_comment\":\"All planned work is terminal.\",\"outcome\":\"completed\"}", "\"outcome_summary\",\"closing_comment\"", "phase" },
    ...
    { "phase.comment_add", "comment", "ongoing execution", "Add a comment to a phase.", "id; body", "comment_type", "body must be non-empty.", "The phase must exist.", "Creates a comment and emits comment_added.", "validation_failed, not_found, internal_error.", "result.comment.", "comment.list, phase.history", "{\"id\":1,\"body\":\"Phase implementation is underway.\",\"comment_type\":\"progress\"}", "\"body\"", "comment" },
    ...
    { "phase.get", "phase", "inspect", "Fetch one phase.", "id", "none", "id must be a positive integer.", "Phase must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "result.phase.", "phase.list, phase.history", "{\"id\":1}", "", "phase" },
    { "phase.history", "phase", "audit/history", "Read audit events for one phase.", "id", "since", "since must be a positive integer when provided.", "Phase must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "result.events.", "event.list, phase.get", "{\"id\":1,\"since\":1}", "", "events" },
    ...
    { "phase.list_tasks", "task", "inspect", "List tasks in a phase.", "id or phase_id", "status, limit, offset", "id and phase_id must be positive; if both are supplied they must match; status must be a task status.", "Phase must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "result.tasks.", "task.list, phase.get", "{\"id\":1,\"status\":\"todo\"}", "", "tasks" },
    { "phase.lookup", "phase", "inspect", "Resolve a phase id from its uid or label+plan scope.", "uid OR (label and plan_id)", "none", "uid and label must be non-empty strings; label requires plan_id scope; id is not accepted (use phase.get for id-based fetches).", "Phase must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "result.id.", "phase.get, plan.lookup, task.lookup", "{\"label\":\"implementation\",\"plan_id\":1}", "", "id" },
    { "phase.move", "phase", "ongoing execution", "Move a phase to another sequence number.", "id; sequence_no", "none", "sequence_no must be a positive integer.", "Phase must exist.", "Reorders phases and emits phase_moved when changed.", "validation_failed, not_found, conflict, internal_error.", "result.phase.", "phase.list", "{\"id\":1,\"sequence_no\":2}", "\"sequence_no\"", "phase" },
    { "phase.progress", "phase", "inspect", "Summarize task status counts for a phase.", "id", "none", "", "Phase must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "status counts, total, completion_percentage.", "phase.list_tasks, task.list", "{\"id\":1}", "", "phase_id, plan_id, counts, total, completion_percentage" },
    { "phase.reopen", "phase", "ongoing execution", "Reopen a completed or canceled phase.", "id", "none", "", "Phase must exist and be completed or canceled.", "Sets phase to open and emits phase_reopened.", "validation_failed, not_found, conflict, internal_error.", "result.phase.", "phase.close", "{\"id\":1}", "", "phase" },
    ...
    { "phase.update", "phase", "ongoing execution", "Update editable phase fields.", "id; plus at least one updated field", "title, summary, description, owner, target_start_date, target_end_date", "title must remain non-empty; nullable fields may be null.", "Phase must exist.", "Updates phase and emits phase_updated.", "validation_failed, not_found, conflict, internal_error.", "result.phase.", "phase.get, phase.history", "{\"id\":1,\"summary\":\"Updated phase scope\"}", "", "phase" },
    ...
    { "plan.archive", "plan", "close", "Archive a completed or canceled plan.", "id", "none", "id must be positive.", "Plan must exist and be completed or canceled.", "Sets status archived and emits plan_archived.", "validation_failed, not_found, conflict, internal_error.", "result.plan.", "plan.close, plan.reopen", "{\"id\":1}", "", "plan" },
    { "plan.close", "plan", "close", "Close a plan as completed or canceled.", "id; outcome_summary; closing_comment", "outcome, lessons_learned, open_items_summary, followup_needed", "outcome is completed or canceled and defaults to completed; id must be positive; outcome_summary and closing_comment must be non-empty strings.", "Plan must exist and be open, in_progress, or paused.", "Updates plan status and emits plan_closed.", "validation_failed, not_found, conflict, internal_error.", "result.plan.", "plan.reopen, plan.archive, plan.history", "{\"id\":1,\"outcome\":\"completed\",\"outcome_summary\":\"All phases delivered.\",\"closing_comment\":\"Approved by stakeholder.\"}", "\"outcome_summary\",\"closing_comment\"", "plan" },
    ...
    { "plan.get", "plan", "inspect", "Fetch one plan by id.", "id", "none", "id must be positive. To resolve a uid, label, or code into an id, call plan.lookup first.", "Plan must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "result.plan.", "plan.list, plan.history", "{\"id\":1}", "", "plan" },
    { "plan.history", "plan", "audit/history", "Read audit events for one plan.", "id", "since", "id must be positive; since is a positive event id.", "Plan must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "result.events.", "event.list, plan.get", "{\"id\":1,\"since\":1}", "", "events" },
    ...
    { "plan.progress", "plan", "inspect", "Summarize task status counts for a plan.", "id", "none", "id must be positive.", "Plan must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "status counts, total, completion_percentage.", "task.list, phase.progress", "{\"id\":1}", "", "plan_id, counts, total, completion_percentage" },
    { "plan.reopen", "plan", "ongoing execution", "Reopen a completed, canceled, or archived plan.", "id", "none", "id must be positive.", "Plan must exist and be completed, canceled, or archived.", "Sets status open and emits plan_reopened.", "validation_failed, not_found, conflict, internal_error.", "result.plan.", "plan.close, plan.archive", "{\"id\":1}", "", "plan" },
    { "plan.update", "plan", "ongoing execution", "Update editable plan fields.", "id; plus at least one updated field", "title, summary, description, priority, owner, target_date, tags, version_label", "title must remain non-empty; priority and tags are validated; nullable fields may be null; id must be positive.", "Plan must exist.", "Updates plan and emits plan_updated.", "validation_failed, not_found, conflict, internal_error.", "result.plan.", "plan.get, plan.history", "{\"id\":1,\"summary\":\"Updated release scope\",\"priority\":\"critical\"}", "", "plan" },
    { "project.get", "project", "inspect", "Read the workspace-level project metadata (name, description, timestamps).", "none", "none", "params must be an object, normally empty.", "Initialized workspace (the project row is created by migration 0004).", "No business data changes.", "internal_error.", "result.project.", "project.update, project.history, workspace.context_get", "{}", "", "project" },
    { "project.history", "project", "audit/history", "List project-level activity: project, tool, and env_var changes plus major plan lifecycle events. Phase, task, and comment events are excluded.", "none", "limit, offset", "limit is 1..500 (defaults to 100); offset is >=0 (defaults to 0).", "none.", "No business data changes.", "validation_failed, internal_error.", "result.events, result.limit, result.offset, result.has_more, result.total_count.", "event.list, plan.history", "{\"limit\":50}", "", "events, limit, offset, has_more, total_count" },
    { "project.update", "project", "ongoing execution", "Update the project's name and/or description.", "at least one of name or description", "name, description", "name and description must be non-empty when provided; at least one of the two must be supplied.", "Initialized workspace.", "Updates project metadata and emits project_updated.", "validation_failed, internal_error.", "result.project.", "project.get, project.history", "{\"name\":\"my-app\",\"description\":\"What this project does in one line.\"}", "", "project" },
    { "task.assign", "task", "ongoing execution", "Assign a task.", "id; assignee", "none", "assignee must be non-empty.", "Task must exist.", "Updates task and emits task_updated.", "validation_failed, not_found, internal_error.", "result.task.", "task.unassign, task.get", "{\"id\":1,\"assignee\":\"alex\"}", "\"assignee\"", "task" },
    { "task.cancel", "task", "cancel", "Cancel a task with terminal resolution and closure memory.", "id; resolution; closing_comment or comment", "outcome_summary, reason_code, reason_text, lessons_learned, open_items_summary, followup_needed", "resolution must be canceled, not_planned, discarded, or duplicate; closure memory must be non-empty.", "Task must exist and be eligible to move to canceled.", "Sets status canceled, writes closure record, may write closure comment, emits task_canceled.", "validation_failed, not_found, conflict, internal_error.", "result.task.", "task.mark_duplicate, task.replace, closure.get", "{\"id\":1,\"resolution\":\"not_planned\",\"closing_comment\":\"No longer in scope.\",\"outcome_summary\":\"Canceled before implementation.\"}", "\"resolution\"", "task" },
    { "task.close", "task", "close", "Close a task as completed with closure memory.", "id; outcome_summary; closing_comment", "lessons_learned, open_items_summary, followup_needed, commit_sha, dirty, files_changed, validations_run, decisions", "closure text must be non-empty; followup_needed and dirty are boolean; commit_sha must be non-empty when provided; files_changed must be an array of strings or null; validations_run must be an array of {cmd, status} objects with non-empty strings; decisions must be an array of non-empty strings or null.", "Task must exist, be eligible to move to done, and not already have a resolution.", "Sets status done, resolution completed, writes closure record (including any commit_sha / dirty / files_changed / validations_run / decisions fields supplied by the client), emits task_closed.", "validation_failed, not_found, conflict, internal_error.", "result.task.", "closure.get, task.reopen", "{\"id\":1,\"outcome_summary\":\"Implemented and tested.\",\"closing_comment\":\"Ready for review.\",\"commit_sha\":\"abc1234\",\"dirty\":false,\"files_changed\":[\"src/foo.c\"],\"validations_run\":[{\"cmd\":\"make test\",\"status\":\"passed\"}],\"decisions\":[\"Used the X approach\"]}", "\"outcome_summary\",\"closing_comment\"", "task" },
    { "task.comment_add", "comment", "ongoing execution", "Add a comment to a task.", "id; body", "comment_type", "body must be non-empty.", "Task must exist.", "Creates a comment and emits comment_added.", "validation_failed, not_found, internal_error.", "result.comment.", "comment.list, task.history via event.list", "{\"id\":1,\"body\":\"Starting implementation now.\",\"comment_type\":\"progress\"}", "\"body\"", "comment" },
    { "task.create", "task", "create/bootstrap", "Create a task in a plan.", "plan_id, title", "phase_id, parent_task_id, summary, description, status, priority, task_type, origin_type, assignee, due_date, target_start_date, estimate, origin_ref_type, origin_ref_id, origin_task_id, label", "title must be non-empty; status can only be todo on create; priority, task_type, and origin_type use public enums; label is auto-generated from title if omitted.", "Plan must exist; phase, parent, and origin task must belong to the same plan when provided.", "Creates task with auto-generated uid and label, emits task_created.", "validation_failed, not_found, conflict, internal_error.", "result.task.", "task.transition, task.defer, task.close", "{\"plan_id\":1,\"phase_id\":1,\"title\":\"Implement feature\",\"priority\":\"high\",\"task_type\":\"task\",\"origin_type\":\"planned\"}", "\"plan_id\",\"title\"", "task" },
    ...
    { "task.defer", "task", "defer", "Defer active work while preserving why it moved out of current scope.", "id", "deferred_until, reason_code, reason_text", "At least one of reason_code or reason_text is required.", "Task must exist and be todo, in_progress, blocked, or already deferred.", "Sets status deferred, stores deferral fields, emits task_deferred.", "validation_failed, not_found, conflict, internal_error.", "result.task.", "task.transition, task.cancel, task.replace", "{\"id\":1,\"deferred_until\":\"2026-05-01\",\"reason_code\":\"waiting-on-input\",\"reason_text\":\"Need product decision before continuing.\"}", "", "task" },
    { "task.get", "task", "inspect", "Fetch one task.", "id", "none", "id must be a positive integer.", "Task must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "result.task.", "task.list, comment.list, closure.get", "{\"id\":1}", "", "task" },
    ...
    { "task.reopen", "task", "ongoing execution", "Reopen a terminal task.", "id", "none", "", "Task must exist, be terminal, not resolution=replaced, and if it belongs to a phase that phase must not be terminal (reopen the phase first).", "Sets task to todo, clears terminal fields, emits task_reopened.", "validation_failed, not_found, conflict, internal_error.", "result.task.", "task.close, task.cancel", "{\"id\":1}", "", "task" },
    { "task.replace", "task", "replace", "Cancel an old task as replaced and create the replacement task.", "id; title; closing_comment or comment", "phase_id, parent_task_id, summary, description, priority, task_type, assignee, due_date, target_start_date, estimate, reason_code, reason_text, relation_notes, outcome_summary, lessons_learned, open_items_summary, followup_needed", "replacement title must be non-empty; closure memory must be non-empty; priority and task_type are enums.", "Old task must be replaceable; replacement phase/parent must be in same plan.", "Cancels old task with resolution replaced, creates new task with origin_type replacement, creates replaces relation, writes closure record.", "validation_failed, not_found, conflict, internal_error.", "result.old_task, result.new_task, result.relation_id.", "task.cancel, task.close, closure.get", "{\"id\":1,\"title\":\"Implement revised feature\",\"closing_comment\":\"Original scope changed.\",\"outcome_summary\":\"Replaced with narrower task.\",\"priority\":\"high\"}", "\"title\"", "old_task, new_task, relation_id" },
    { "task.set_origin", "task", "ongoing execution", "Set origin metadata for a task.", "id; origin_type", "origin_ref_type, origin_ref_id, origin_task_id", "origin_type must be planned, addendum, discovered, replacement, carryover, or external_request.", "Task must exist; origin_task_id must exist in same plan when provided.", "Updates origin fields and emits task_updated.", "validation_failed, not_found, conflict, internal_error.", "result.task.", "task.get, task.list", "{\"id\":1,\"origin_type\":\"discovered\",\"origin_ref_type\":\"review\",\"origin_ref_id\":\"PR-12\"}", "\"origin_type\"", "task" },
    { "task.set_priority", "task", "ongoing execution", "Set task priority.", "id; priority", "none", "priority must be low, medium, high, or critical.", "Task must exist.", "Updates priority and emits task_updated.", "validation_failed, not_found, internal_error.", "result.task.", "task.update, task.list", "{\"id\":1,\"priority\":\"critical\"}", "\"priority\"", "task" },
    ...
    { "task.set_type", "task", "ongoing execution", "Set task type.", "id; task_type", "none", "task_type must be task, research, bug, decision, review, or documentation.", "Task must exist.", "Updates task_type and emits task_updated.", "validation_failed, not_found, internal_error.", "result.task.", "task.update, task.list", "{\"id\":1,\"task_type\":\"bug\"}", "\"task_type\"", "task" },
    { "task.transition", "task", "ongoing execution", "Move a task through non-terminal generic statuses.", "id; status", "none", "status must be a valid task status; dedicated operations are required for deferred, done, and canceled terminal flows.", "Task must exist and transition must be allowed.", "Updates status and emits task_status_changed.", "validation_failed, not_found, conflict, internal_error.", "result.task.", "task.defer, task.close, task.cancel", "{\"id\":1,\"status\":\"in_progress\"}", "\"status\"", "task" },
    { "task.unassign", "task", "ongoing execution", "Clear a task assignee.", "id", "none", "", "Task must exist.", "Clears assignee and emits task_updated.", "validation_failed, not_found, internal_error.", "result.task.", "task.assign", "{\"id\":1}", "", "task" },
    ...
    { "task.update", "task", "ongoing execution", "Update editable task fields without changing semantic terminal state.", "id; plus at least one updated field", "title, summary, description, due_date, target_start_date, estimate, blocked_reason, reason_code, reason_text", "title must remain non-empty; nullable fields may be null.", "Task must exist.", "Updates task and emits task_updated.", "validation_failed, not_found, internal_error.", "result.task.", "task.set_priority, task.set_type, task.transition", "{\"id\":1,\"summary\":\"Updated implementation details\",\"estimate\":\"1d\"}", "", "task" },
    { "tool.add", "tool", "create/bootstrap", "Register a tool the project requires (runtime, CLI, library).", "name", "version_constraint, purpose, install_hint, required", "name must be non-empty; version_constraint, purpose, and install_hint must be non-empty when provided; required is boolean and defaults to true. version_constraint is stored as opaque text — ipman does not parse semver.", "The active row must use a unique name.", "Creates a tool and emits tool_added on the project.", "validation_failed, conflict, internal_error.", "result.tool.", "tool.list, tool.update, tool.invalidate", "{\"name\":\"node\",\"version_constraint\":\">=20\",\"purpose\":\"runtime\",\"install_hint\":\"mise use node@20\"}", "\"name\"", "tool" },
    { "tool.invalidate", "tool", "ongoing execution", "Soft-invalidate a tool while preserving audit history.", "id", "none", "id must be a positive integer.", "The tool must exist and not already be invalidated.", "Sets invalidated fields and emits tool_invalidated on the project.", "validation_failed, not_found, conflict, internal_error.", "result.tool.", "tool.list, tool.update", "{\"id\":1}", "\"id\"", "tool" },
    { "tool.list", "tool", "inspect", "List the tools required by the project.", "none", "include_invalidated", "include_invalidated must be boolean.", "none.", "No business data changes.", "validation_failed, internal_error.", "result.tools, result.include_invalidated, result.has_more, result.total_count.", "tool.add, tool.update, tool.invalidate", "{\"include_invalidated\":false}", "", "tools, include_invalidated, has_more, total_count" },
...
static const EntitySpec k_entity_specs[] = {
    { "plan", "Top-level container for a body of work.", "id (canonical), label (semantic slug unique globally), code, title, summary, description, status, priority, owner, target_date, tags, version_label, timestamps.", "Status is one of open, in_progress, paused, completed, canceled, archived. id is the canonical selector. Plans are preserved and archived instead of being deleted.", "plan.create, plan.get, plan.list, plan.update, plan.close, plan.archive, plan.reopen, plan.history, plan.progress, plan.comment_add.", "Pass id to non-lookup ops. Use plan.lookup to translate uid/label/code into id. Use plan operations for state changes instead of editing SQLite." },
    { "phase", "Ordered stage inside a plan.", "id (canonical), label (semantic slug unique per plan), plan_id, title, summary, description, status, sequence_no, local_seq, owner, target_start_date, target_end_date, timestamps.", "A phase belongs to one plan and has a unique sequence number inside that plan. Closing a phase requires terminal tasks. id is the canonical selector.", "phase.create, phase.get, phase.list, phase.update, phase.move, phase.close, phase.reopen, phase.history, phase.progress, phase.list_tasks, phase.comment_add.", "Pass id to non-lookup ops. Use phase.lookup with uid or with label+plan_id to translate human-readable handles into id." },
    { "task", "Primary operational unit of work.", "id (canonical), label (semantic slug unique per plan), plan_id, phase_id, parent_task_id, title, summary, description, status, resolution, priority, task_type, origin_type, assignee, deferred_until, blocked_reason, reason fields, due dates, origin metadata, local_seq, timestamps.", "status, resolution, and origin_type are separate. status is current workflow position; resolution explains terminal outcome; origin_type records why the task exists. id is the canonical selector. Replacement links live in task_relations.", "task.create, task.get, task.list, task.update, task.transition, task.defer, task.cancel, task.replace, task.close, task.reopen, task.assign, task.unassign, task.set_priority, task.set_type, task.set_origin, task.link_external, task.link_dependency, task.unlink_dependency, task.mark_duplicate, task.comment_add.", "Use semantic operations for defer, cancel, replace, and close. Pass id to non-lookup ops; use task.lookup with uid or with label+plan_id to translate handles into id. Comments are context, not state." },
    ...
    { "instruction", "Durable guidance attached to a plan, phase, task, or the project itself.", "id, entity_type, entity_id, instruction_type, priority, body, author, created_at, updated_at, invalidated_at, invalidated_by.", "Instructions represent active operating guidance. Project-scoped instructions (entity_type='project', entity_id=1) are repository-wide and survive plan lifecycle; they surface in workspace.context_get under context.project.instructions. Invalidated instructions remain queryable for history but are excluded from normal lists. The priority field ('critical' or 'normal', default 'normal') is only meaningful for project-scoped instructions and signals which rules must be followed without exception versus which are standing preferences.", "instruction.add, instruction.list, instruction.update, instruction.invalidate.", "Use instructions for constraints and standing guidance agents should read before acting; use comments for conversational history. Reach for project-scoped instructions when the rule is repository-wide; reach for plan/phase/task scope when it is context-specific. Mark an instruction priority='critical' when breaking it would produce a wrong or harmful outcome (wrong language, broken design system, security violation); leave priority='normal' for style and convention preferences." },
    { "project", "Workspace-level metadata describing what this project is and what it requires.", "id (always 1), name, description, created_at, updated_at. Tools and env_vars belong to the project but live in their own tables.", "The project table is single-row by construction (CHECK id = 1) and is initialized by migration 0004 with name='unnamed'. Use project.update to set the actual name and description for this repository.", "project.get, project.update, project.history.", "Use the project entity to record what this codebase is for, who reads it, and what it needs to run. Tools and env_vars hang off the project, not off any individual plan, because they describe the codebase as a whole." },
    { "tool", "Tool, runtime, or library the project requires.", "id, name, version_constraint, purpose, install_hint, required, author, created_at, updated_at, invalidated_at, invalidated_by.", "Tools are project-scoped (workspace-level). The active row per name is unique. version_constraint is stored as opaque text — ipman never parses semver. Invalidated rows remain queryable for history.", "tool.add, tool.list, tool.update, tool.invalidate.", "Use tools to declare what the project needs to run (compilers, CLIs, libraries). ipman is the registry; use mise/asdf/devbox to actually install or verify versions." },
    { "env_var", "Environment variable the project requires, recorded by metadata only.", "id, name, purpose, example, required, sensitive, author, created_at, updated_at, invalidated_at, invalidated_by. There is intentionally no value column — ipman is a registry of requirements, never a secret store.", "name must match ^[A-Z_][A-Z0-9_]*$. The active row per name is unique. When sensitive=true, the example field is masked in env_var.list and workspace.context_get unless the caller passes reveal=true.", "env_var.add, env_var.list, env_var.update, env_var.invalidate.", "Use env_vars to document what the project needs in its environment. Store actual values in your secret manager (.env, vault, CI secrets), never in ipman." },
    { "closure_record", "Durable memory produced when work closes.", "id, entity_type, entity_id, closure_status, resolution, outcome_summary, closing_comment, lessons_learned, open_items_summary, followup_needed, author, event_id, commit_sha (optional), dirty (optional boolean), files_changed (optional array), validations_run (optional array of {cmd, status}), decisions (optional array of strings).", "Closure records are linked to events and preserve handoff memory after terminal operations. The git-context fields (commit_sha, dirty, files_changed) and the structured-evidence fields (validations_run, decisions) are populated only on task.close when the client sends them; cancel / replace / duplicate paths leave them absent.", "closure.get, task.close, task.cancel, task.replace, task.mark_duplicate, phase.close.", "Use closure.get before resuming old work or reconstructing why something ended." },
    ...
    { "workspace", "Ambient state for the ipman home directory itself — bootstrap, smoke checks, and regeneration of the agent knowledge pack.", "IPMAN_HOME path, db file, and the generated artifacts under it (entrypoints, indexes, entities, operations, schemas, fingerprints).", "An effective IPMAN_HOME must be initialized and writable. Workspace operations do not touch business data (plans, phases, tasks, comments).", "noop, workspace.refresh_agent_docs.", "Use noop as a liveness check after init or environment changes. Run workspace.refresh_agent_docs after upgrading ipman or modifying operation specs to keep the on-disk knowledge pack current." },
    { "workspace_context", "Workspace-level focus state for agent handoff.", "workspace_context.active_plan_id plus per-plan current_phase_id/current_task_id in plan_contexts.", "The active plan is global to the workspace, but plan.activate also records a git branch → plan binding (branch_contexts). workspace.context_get uses the current branch's binding when one exists, falling back to the global active_plan_id. Current phase/task are explicit per-plan cursors and are not inferred from statuses.", "workspace.context_get, plan.activate, plan.deactivate, phase.set_current, phase.clear_current, task.set_current, task.clear_current, workspace.list_branch_bindings, workspace.unbind_branch.", "Use context operations to resume work; use plan/phase/task status operations for workflow state." },
};
...
static char *render_start_here(const char *home, const char *db_path,
    ...
    if (buf_appendf(&b,
        ...
        "- `ipman --current <selector>`: set the current task or phase (auto-detected).\n"
        "- `ipman --start <selector>`: mark a task `in_progress`.\n"
        "- `ipman --close <selector> --summary <text> --comment <text>`: close a task with closure record (auto-captures git state when run inside a repo).\n"
        "- `ipman --cancel <selector> --summary <text> --comment <text>`: cancel a task with closure record.\n"
        "- `ipman --defer <selector> --reason-text <text>`: defer a task with reason.\n"
        "- `ipman --dry-run`: combine with any write verb to print the JSON envelope that would be sent and exit without touching the DB.\n\n"
        "Run `ipman --usage` for the complete CLI surface.\n\n"
        "## JSON Protocol\n\n"
        "For operations without a CLI wrapper (creating plans, decomposing into phases, threading comments, exporting), invoke ipman as a JSON-in / JSON-out tool by sending one JSON request on stdin and reading one JSON response on stdout:\n\n"
        "```sh\n"
    ...
    if (buf_append(&b,
        "## Entity Identity\n\n"
        "Every entity carries multiple identifiers with distinct roles:\n\n"
        "- **`id`**: canonical input and output identifier. Workspace-scoped integer (e.g. `42`). Required by every non-lookup op (`*.get`, `*.update`, lifecycle ops, etc.). Returned on every API response.\n"
        "- **`label`**: human-readable slug (e.g. `implement-login`). Auto-derived from `title` or supplied at creation. Returned on responses for human display. Not accepted as a selector by non-lookup ops — use `*.lookup` to translate it into `id`.\n"
        "- **`code`** (plans only): user-supplied plan shorthand (e.g. `REL-001`, auto-assigned as `P<id>`). Stored, used as a `plan.lookup` key and `plan.activate` selector, and returned where workspace context or exports need the display handle.\n"
        "- **`uid`**: storage-only identifier (e.g. `task_42`, `phase_3`, `plan_1`). Persisted in the database and emitted by `plan.export` for archival round-trip, but not surfaced in ordinary entity API responses.\n"
        ...
        "Selector rules:\n\n"
        "1. Non-lookup ops (`*.get`, `*.update`, lifecycle ops) accept `id` only. Pass the integer directly.\n"
        "2. Lookup ops (`plan.lookup`, `phase.lookup`, `task.lookup`) translate human-readable handles into `id`. `plan.lookup` accepts `uid`, `label`, or `code`; `phase.lookup` and `task.lookup` accept `uid` or `label` (label requires `plan_id` scope). Lookup ops return `{id: N}` only.\n"
        "3. `plan.activate` is the single non-lookup op that still accepts `code` (alongside `id`), since it is the session entry point.\n"
        "4. `phase.list_tasks` accepts `id` or `phase_id` (alias). When both are supplied they must match.\n"
        "5. Generated request JSON schemas express allowed params, primitive types/enums, required scalar fields, and closure/update alternatives. Runtime validation remains the authority for database existence, state transitions, and cross-field business rules.\n\n"
        "## Workspace Context\n\n"
...
static char *render_concepts_doc(const char *generated_at) {
    ...
    if (buf_append(&b,
        ...
        "- Instruction: durable guidance attached to a plan, phase, task, or the project itself. Project-scoped instructions are repository-wide and survive plan lifecycle. Priority='critical' instructions are non-negotiable; priority='normal' are standing preferences.\n"
...
static char *render_enums_doc(const char *generated_at) {
    ...
    if (buf_append(&b,
        ...
        "- Instruction priority: `critical` (non-negotiable, must be followed without exception), `normal` (standing preference). Only applies to project-scoped instructions.\n"
...
static const char *selector_note_for_op(const char *name) {
    if (op_uses_task_selector(name) || op_uses_phase_selector(name)) {
        return "Selector contract: accepts `id` only. To resolve a `uid` or `label` to an `id`, call the matching `*.lookup` op first.";
    }
    if (op_uses_phase_or_phase_id_selector(name)) {
        return "Selector contract: accepts `id` or `phase_id` (alias). When both are supplied they must match.";
    }
    if (op_uses_plan_selector(name)) {
        return "Selector contract: accepts `id` only. To resolve a `uid`, `label`, or `code` to an `id`, call `plan.lookup` first.";
    }
    if (strcmp(name, "plan.activate") == 0) {
        return "Selector contract: requires exactly one of `id` or `code`. `plan.activate` is the only non-lookup op that still accepts `code` directly, since it is the session entry point.";
    }
    if (op_uses_plan_lookup_selector(name)) {
        return "Selector contract: accepts `uid`, `label`, or `code`. `id` is not accepted — use `plan.get` for id-based fetches. Returns `{id: N}` for the resolved plan.";
    }
    if (op_uses_child_lookup_selector(name)) {
        return "Selector contract: accepts `uid`, or `label` with `plan_id` scope. `id` is not accepted — use the corresponding `*.get` op for id-based fetches. Returns `{id: N}` for the resolved phase or task.";
    }
    return NULL;
}
...
static int append_schema_rules(Buf *b, const OperationSpec *spec) {
    ...
    if (op_uses_phase_selector(spec->name) ||
        op_uses_task_selector(spec->name) ||
        op_uses_plan_selector(spec->name) ||
        strcmp(spec->name, "phase.set_current") == 0 ||
        strcmp(spec->name, "task.set_current") == 0) {
        static const char * const id_selector[] = {
            "\"id\"",
            NULL,
        };
        if (append_anyof_required_rule(b, &rule_count, id_selector) != 0) {
            return -1;
        }
    }

    if (op_uses_phase_or_phase_id_selector(spec->name)) {
...
static int render_param_schema(Buf *b, const OperationSpec *spec,
    ...
    if (strcmp(name, "dirty") == 0) {
        return buf_append(b, "{\"type\":[\"boolean\",\"null\"]}");
    }
    if (strcmp(name, "tags") == 0 ||
        strcmp(name, "files_changed") == 0) {
        return buf_append(b, "{\"type\":[\"array\",\"null\"],\"items\":{\"type\":\"string\"}}");
    }
    if (strcmp(name, "decisions") == 0) {
        return buf_append(b, "{\"type\":[\"array\",\"null\"],\"items\":{\"type\":\"string\",\"minLength\":1}}");
    }
    if (strcmp(name, "validations_run") == 0) {
        return buf_append(b,
...
static char *render_workflow_doc(const char *name, const char *generated_at) {
    ...
    if (strcmp(name, "agent-handoff") == 0) {
        if (buf_append(&b,
            ...
            "Recommended sequence: `workspace.context_get` first — its result includes the project instructions block; apply any `priority='critical'` instructions immediately as non-negotiable constraints before proceeding. Then `plan.list` to find active plans, `instruction.list` for plan/phase-scoped guidance, `task.list` with `pending:true` for open work, `event.list` or `plan.history` for recent changes, `comment.list` for current notes, and `closure.get` for terminal memory.\n\n"
...
static char *render_best_practices_doc(const char *generated_at) {
    ...
    if (buf_append(&b,
        ...
        "## 1. Pass `id` everywhere; use `*.lookup` once to resolve names\n\n"
        "`id` is the canonical input and output identifier in v2. Every non-lookup op accepts `id` only. "
        "When you have a human-readable handle — a `label`, a plan `code`, or a stored `uid` — call the matching `*.lookup` op exactly once and reuse the returned `id`. "
        "`plan.lookup` accepts `uid`, `label`, or `code`; `phase.lookup` and `task.lookup` accept `uid` or `label` (label requires `plan_id` scope). "
        "`plan.activate` is the single exception that still accepts `code` directly alongside `id`, since it is the session entry point. "
        "Cache the resolved `id` for the rest of the workflow rather than re-resolving on every call.\n\n"
    ...
    if (buf_append(&b,
        ...
        "1. `workspace.context_get` — recover active plan and current phase/task cursors, and read the project instructions block (`result.context.project.instructions`). **Before proceeding**: apply all instructions with `priority='critical'` as non-negotiable constraints for this session. Apply `priority='normal'` instructions as standing preferences.\n"
