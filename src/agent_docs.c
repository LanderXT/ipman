...
static const OperationSpec k_operation_specs[] = {
    ...
    { "instruction.add", "instruction", "ongoing execution", "Add durable guidance to a plan, phase, task, or the project itself.", "entity_type, entity_id, body", "instruction_type", "entity_type must be plan, phase, task, or project; entity_id must be positive; for project, entity_id must be 1 (single-row table); body and instruction_type must be non-empty strings when provided.", "The referenced entity must exist.", "Creates an instruction and emits instruction_added on the referenced entity.", "validation_failed, not_found, internal_error.", "result.instruction.", "instruction.list, instruction.update, instruction.invalidate", "{\"entity_type\":\"project\",\"entity_id\":1,\"body\":\"Apache-2.0 in all source headers.\",\"instruction_type\":\"constraint\"}", "\"entity_type\",\"entity_id\",\"body\"", "instruction" },
