...
static const OperationSpec k_operation_specs[] = {
    ...
    { "workspace.context_get", "workspace_context", "inspect", "Read the active plan, current phase/task cursor, and the project block (name, description, tools, env_vars) for this workspace.", "none", "reveal", "reveal must be boolean and defaults to false. When false, env_vars marked sensitive return example=\"[sensitive]\" or null instead of the stored example.", "Initialized workspace.", "No business data changes.", "validation_failed, internal_error.", "result.context (includes context.project with tools and env_vars).", "plan.activate, phase.set_current, task.set_current, project.get, tool.list, env_var.list", "{\"reveal\":false}", "", "context" },
