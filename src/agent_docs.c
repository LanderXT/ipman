...
static const OperationSpec k_operation_specs[] = {
    ...
    { "plan.close", "plan", "close", "Close a plan as completed or canceled.", "id", "outcome, outcome_summary, closing_comment, lessons_learned, open_items_summary, followup_needed", "outcome is completed or canceled and defaults to completed; id must be positive.", "Plan must exist and be open, in_progress, or paused.", "Updates plan status and emits plan_closed.", "validation_failed, not_found, conflict, internal_error.", "result.plan.", "plan.reopen, plan.archive, plan.history", "{\"id\":1,\"outcome\":\"completed\"}", "", "plan" },
    ...
    { "plan.get", "plan", "inspect", "Fetch one plan by id.", "id", "none", "id must be positive.", "Plan must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "result.plan.", "plan.list, plan.history", "{\"id\":1}", "\"id\"", "plan" },
    { "plan.history", "plan", "audit/history", "Read audit events for one plan.", "id", "since", "id must be positive; since is a positive event id.", "Plan must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "result.events.", "event.list, plan.get", "{\"id\":1,\"since\":1}", "\"id\"", "events" },
    ...
    { "plan.progress", "plan", "inspect", "Summarize task status counts for a plan.", "id", "none", "id must be positive.", "Plan must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "status counts, total, completion_percentage.", "task.list, phase.progress", "{\"id\":1}", "\"id\"", "plan_id, counts, total, completion_percentage" },
    { "plan.reopen", "plan", "ongoing execution", "Reopen a completed, canceled, or archived plan.", "id", "none", "id must be positive.", "Plan must exist and be completed, canceled, or archived.", "Sets status open and emits plan_reopened.", "validation_failed, not_found, conflict, internal_error.", "result.plan.", "plan.close, plan.archive", "{\"id\":1}", "\"id\"", "plan" },
    { "plan.update", "plan", "ongoing execution", "Update editable plan fields.", "id; plus at least one updated field", "title, summary, description, priority, owner, target_date, tags, version_label", "title must remain non-empty; priority and tags are validated; nullable fields may be null; id must be positive.", "Plan must exist.", "Updates plan and emits plan_updated.", "validation_failed, not_found, conflict, internal_error.", "result.plan.", "plan.get, plan.history", "{\"id\":1,\"summary\":\"Updated release scope\",\"priority\":\"critical\"}", "\"id\"", "plan" },
