...
static const OperationSpec k_operation_specs[] = {
    ...
    { "plan.archive", "plan", "close", "Archive a completed or canceled plan.", "id", "none", "id must be positive; code, uid, and label must be non-empty strings.", "Plan must exist and be completed or canceled.", "Sets status archived and emits plan_archived.", "validation_failed, not_found, conflict, internal_error.", "result.plan.", "plan.close, plan.reopen", "{\"code\":\"REL-001\"}", "", "plan" },
    ...
    { "plan.history", "plan", "audit/history", "Read audit events for one plan.", "id", "since", "id must be positive; code, uid, and label must be non-empty; since is a positive event id.", "Plan must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "result.events.", "event.list, plan.get", "{\"code\":\"REL-001\",\"since\":1}", "", "events" },
    ...
    { "plan.progress", "plan", "inspect", "Summarize task status counts for a plan.", "id", "none", "id must be positive; code, uid, and label must be non-empty strings.", "Plan must exist.", "No business data changes.", "validation_failed, not_found, internal_error.", "status counts, total, completion_percentage.", "task.list, phase.progress", "{\"code\":\"REL-001\"}", "", "plan_id, counts, total, completion_percentage" },
    { "plan.reopen", "plan", "ongoing execution", "Reopen a completed, canceled, or archived plan.", "id", "none", "id must be positive; code, uid, and label must be non-empty strings.", "Plan must exist and be completed, canceled, or archived.", "Sets status open and emits plan_reopened.", "validation_failed, not_found, conflict, internal_error.", "result.plan.", "plan.close, plan.archive", "{\"code\":\"REL-001\"}", "", "plan" },
    { "plan.update", "plan", "ongoing execution", "Update editable plan fields.", "id; plus at least one updated field", "title, summary, description, priority, owner, target_date, tags, version_label", "title must remain non-empty; priority and tags are validated; nullable fields may be null; id must be positive; code, uid, and label must be non-empty strings.", "Plan must exist.", "Updates plan and emits plan_updated.", "validation_failed, not_found, conflict, internal_error.", "result.plan.", "plan.get, plan.history", "{\"code\":\"REL-001\",\"summary\":\"Updated release scope\",\"priority\":\"critical\"}", "", "plan" },
...
static char *render_start_here(const char *home, const char *db_path,
    ...
    if (buf_append(&b,
        ...
        "- **`code`** (plans only): user-supplied plan shorthand (e.g. `REL-001`, auto-assigned as `P<id>`). Stored, used as a `plan.lookup` key and `plan.activate` selector, but not echoed in API responses.\n"
        "- **`uid`**: storage-only identifier (e.g. `task_42`, `phase_3`, `plan_1`). Persisted in the database and emitted by `plan.export` for archival round-trip, but not surfaced in any API response.\n"
